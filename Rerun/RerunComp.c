...
int autonSelect;	//LCD Autonomous Selecting Variable
//Main Screen Options Array
string mainScreen[5] = {"<- Autonomous ->", "<- Statistics ->", "<- Battery V. ->", "<-  Controls  ->", "<-   Reruns   ->"};
//Autonomous Select Screen Options Array
string autonScreen[8] = {"<- RedSkyrise ->","<-  Red Cube  ->", "<-  Blue Cube  ->", "<-  CubeBackup  ->", "<- EMPTYAUTON ->", "<- EMPTYAUTON ->", "<-  EMPTYAUTON  ->", "<-  EMPTYAUTON  ->"};
...
//Rerun Autonomous Select Screen Options Array
string rerunScreen[9] = {"<-  Rerun001  ->", "<-  Rerun002  ->", "<-  Rerun003  ->", "<-  Rerun004  ->", "<-  Rerun005  ->", "<-  Rerun006  ->", "<-  Rerun007  ->", "<-  Rerun008  ->", "<-  Rerun009  ->"};
...
string rerunFile;			//LCD Temporary String for Rerun Configuration
string tempRerun;				//LCD Temporary String for Rerun Configuration
int CURRENT_SHIFT = 0;
int hookValues[5];
	...
	while(true){
		...
		if(y == 0){					//LCD Main Screen
			...
			while(y == 0){
				...
				upperLim = ARRAY_SIZE(mainscreen)-1;
			...
			while(y == 1){
				...
				upperLim = ARRAY_SIZE(autonScreen)-1;
				if(nLCDButtons == leftbtn){
					z--;
				}
				else if(nLCDButtons == rightbtn){
					...
					if(autonSelect != NULL){
						...
						displayLCDCenteredString(0, "ERROR_AUTONOMOUS");
			...
			while(y == 1){
				...
				upperLim = ARRAY_SIZE(statScreen)-1;
				if(nLCDButtons == leftbtn){
					z--;
				}
				else if(nLCDButtons == rightbtn){
			...
			while(y == 1){
				...
				upperLim = ARRAY_SIZE(controlScreen)-1;
				if(nLCDButtons == leftbtn){
					z--;
				}
				else if(nLCDButtons == rightbtn){
		...
		else if(y == 1 && x == 4){	//LCD Rerun Autonomous Screen
			z = 0;
			while(y == 1){
				upperLim = RCFS_FindLastSlot()-1;
				if(upperLim == -1){
					displayLCDCenteredString(0, "ERROR_NO_RERUN");
					displayLCDCenteredString(1, "AVAILABLE");
				}
				else{
					displayScreen(rerunScreen[z]);
				}
				if(nLCDButtons == leftbtn){
					z--;
				}
					...
					y++;
					if(autonSelect != NULL){
						displayLCDCenteredString(0, "Rerun Autonomous");
						displayLCDCenteredString(1, "Selected");
					}
					else{
						displayLCDCenteredString(0, "ERROR_AUTONOMOUS");
...
task autonomous()
{
	...
	else if(autonSelect >= 8){		//Reruns
		tempRerun = "rerun";
		sprintf(rerunFile, "%s%03d", tempRerun , autonSelect-7);
		int i = 0;
    unsigned char *data;
    int datalength;
